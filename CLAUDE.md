...
- `/` Accueil éditoriale · `/simulateur-debutant` simulateur guidé (`BeginnerSimulator.jsx` ; mise en page **de haut en bas**, 3 zones : ① « Votre situation financière » = `ProfileAdvisor.jsx` + moteur pur `utils/advisor.js` (foyer/revenus/charges/crédits/épargne/horizon/risque → diagnostic budgétaire + plan suggéré **toujours en DCA**, applicable d'un clic ; données perso jamais encodées dans l'URL de partage), ② les 4 étapes courtier→enveloppe→actifs→plan en grille 2 colonnes (versement initial optionnel pour les plans mensuels), ③ le résultat en pleine largeur) · `/simulateur` simulateur avancé (8 stratégies : Lump Sum, DCA, Value Averaging, DCA dynamique, Buy & Hold, Rééquilibrage, Stock Picking, Momentum ; CAGR/Sharpe/MaxDD/vol).
